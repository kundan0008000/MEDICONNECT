    ...
    <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-[#ADADAD]'>
      ...
      <ul className='md:flex items-start gap-5 font-medium hidden'>
        ...
        <NavLink to='/prescription' >
          <li className='py-1'>QR PRESCRIPTION</li>
          <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden' />
        </NavLink>
      ...
      <div className='flex items-center gap-4 '>
        ...
        <div className={`md:hidden ${showMenu ? 'fixed w-full' : 'h-0 w-0'} right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
          ...
          <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
            ...
            <NavLink onClick={() => setShowMenu(false)} to='/prescription' ><p className='px-4 py-2 rounded full inline-block'>QR PRESCRIPTION</p></NavLink>
            <NavLink onClick={() => setShowMenu(false)} to='/ambulance' ><p className='px-4 py-2 rounded full inline-block'>AMBULANCE</p></NavLink>
